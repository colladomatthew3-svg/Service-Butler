...
exports.overpassConnector = void 0;
...
async function timeoutFetch(url, init, timeoutMs = 9000) {
...
exports.overpassConnector = {
    ...
    async normalize(records, input) {
        ...
        return records.map((record, index) => {
            ...
            const occurredAt = toIso(record.updated_at || new Date().toISOString());
            return {
                occurredAt,
    ...
    async healthcheck(input) {
        ...
        const query = String(input.config.query || process.env.OVERPASS_QUERY || "").trim();
        if (!endpoint || !query)
            return { ok: false, detail: "OVERPASS endpoint/query missing" };
