...
export type BookedJobWebhookPayload = {
  tenantId?: string;
  jobId?: string;
...
export type BookedJobWebhookResult = {
  received: boolean;
  duplicate: boolean;
  jobId: string;
  attribution: {
...
function webhookEventKey(payload: Required<Pick<BookedJobWebhookPayload, "tenantId" | "jobId">> & BookedJobWebhookPayload) {
...
async function resolveLegacyAccountIdForTenant({
  supabase,
  tenantId
}: {
...
async function markWebhookEventProcessing({
  ...
  tenantId: string;
  eventId: string;
  payload: BookedJobWebhookPayload;
}) {
...
async function resolveAttribution({
  ...
  explicitSourceEventId: string | null;
...
export async function processBookedJobWebhook({
  supabase,
  payload
}: {
  supabase: SupabaseClient;
  payload: BookedJobWebhookPayload;
}): Promise<BookedJobWebhookResult> {
  ...
  if (jobError) throw new Error(jobError.message || "Failed to upsert job");
  ...
  if (attributionError) throw new Error(attributionError.message || "Failed to upsert job attribution");
