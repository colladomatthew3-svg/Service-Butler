...
async function GET() {
    if ((0, review_mode_1.isDemoMode)() || !feature_flags_1.featureFlags.useV2Reads) {
        return server_1.NextResponse.json({ mode: "compat", metrics: [], opportunities: [], assignments: [], jobs: [] });
    }
    const context = await (0, context_1.getV2TenantContext)();
    if (!context)
        return server_1.NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    (0, rbac_1.assertRole)(context.role, ["ACCOUNT_OWNER", "DISPATCHER", "TECH", "READ_ONLY"]);
    try {
        const data = await (0, dashboard_read_models_1.getFranchiseDashboardReadModel)({
