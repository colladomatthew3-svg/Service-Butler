...
exports.censusConnector = void 0;
...
function classifyMarketRisk(record) {
    ...
    const composite = Math.round(Math.max(0, Math.min(100, olderHousingRatio * 0.35 + renterRate * 0.2 + vacancyRate * 0.1 + severeWeatherExposure * 0.35))) || 40;
...
function rowsFromCensusArray(payload) {
...
exports.censusConnector = {
    ...
    async pull(input) {
        ...
        const defaultEndpoint = "https://api.census.gov/data/2023/acs/acs5" +
        ...
        if (Array.isArray(payload))
            return rowsFromCensusArray(payload);
    ...
    async healthcheck(input) {
        ...
        const stateCode = String(input.config.state_code || process.env.CENSUS_API_STATE || "36").trim();
        const apiKey = String(input.config.api_key || process.env.CENSUS_API_KEY || "").trim();
        const fallbackEndpoint = "https://api.census.gov/data/2023/acs/acs5" +
            `?get=NAME,B25034_010E,B25034_011E,B25003_003E,B25004_001E&for=county:*&in=state:${encodeURIComponent(stateCode)}` +
            (apiKey ? `&key=${encodeURIComponent(apiKey)}` : "");
        const endpoint = String(input.config.endpoint || process.env.CENSUS_API_ENDPOINT || fallbackEndpoint).trim();
        if (!endpoint)
            return { ok: false, detail: "CENSUS API endpoint missing" };
