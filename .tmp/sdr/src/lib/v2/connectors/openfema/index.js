...
exports.openFemaConnector = void 0;
...
function classifyIncidentType(value) {
    const text = value.toLowerCase();
    if (text.includes("flood") || text.includes("coastal storm"))
        return { category: "flood_incident", serviceLines: ["restoration", "plumbing"], catastrophe: 88 };
    ...
    if (text.includes("fire"))
        return { category: "fire_incident", serviceLines: ["restoration"], catastrophe: 86 };
    if (text.includes("winter") || text.includes("freeze"))
        return { category: "freeze_incident", serviceLines: ["plumbing", "hvac"], catastrophe: 78 };
    return { category: "disaster_incident", serviceLines: ["restoration"], catastrophe: 68 };
}
async function timeoutFetch(url, timeoutMs = 9000) {
...
function sourceTermsStatus(input) {
...
exports.openFemaConnector = {
    ...
    async pull(input) {
        ...
        const endpoint = String(input.config.endpoint ||
            ...
            "https://www.fema.gov/api/open/v2/DisasterDeclarationsSummaries?$top=100&$orderby=declarationDate desc").trim();
        if (!endpoint)
            return [];
        ...
        if (Array.isArray(nested)) {
            return nested.filter((row) => Boolean(row && typeof row === "object"));
        }
    ...
    async normalize(records, input) {
        ...
        return records.map((record) => {
            ...
            return {
                ...
                likelyJobType: classification.category === "fire_incident"
                    ? "fire restoration"
    ...
    classify(event) {
        ...
        if (category === "flood_incident")
            return { opportunityType: "storm_restoration", serviceLine: "restoration" };
        if (category === "fire_incident")
            return { opportunityType: "fire_damage_incident", serviceLine: "restoration" };
        if (category === "freeze_incident")
    ...
    async healthcheck(input) {
        ...
        if (hasSample)
            return { ok: true, detail: "sample_records configured" };
        const endpoint = String(input.config.endpoint ||
            process.env.OPENFEMA_API_URL ||
            "https://www.fema.gov/api/open/v2/DisasterDeclarationsSummaries?$top=1").trim();
        if (!endpoint)
            return { ok: false, detail: "OpenFEMA endpoint missing" };
