...
exports.open311Connector = void 0;
...
exports.open311Connector = {
    ...
    async pull(input) {
        ...
        const endpoint = String(input.config.endpoint || process.env.OPEN311_ENDPOINT || "https://data.cityofnewyork.us/resource/erm2-nwe9.json?$limit=100").trim();
        if (!endpoint)
            return [];
        ...
        if (Array.isArray(payload))
            return payload.filter((row) => Boolean(row && typeof row === "object"));
        const wrapped = payload;
        const rows = Array.isArray(wrapped.results) ? wrapped.results : Array.isArray(wrapped.records) ? wrapped.records : [];
        return rows.filter((row) => Boolean(row && typeof row === "object"));
    },
    ...
    classify(event) {
        ...
        if (category === "water_incident")
            return { opportunityType: "flood_incident", serviceLine: "restoration" };
        if (category === "infrastructure_failure")
            return { opportunityType: "infrastructure_incident", serviceLine: "plumbing" };
    ...
    async healthcheck(input) {
        ...
        const endpoint = String(input.config.endpoint || process.env.OPEN311_ENDPOINT || "https://data.cityofnewyork.us/resource/erm2-nwe9.json?$limit=1").trim();
        if (!endpoint)
            return { ok: false, detail: "OPEN311 endpoint missing" };
