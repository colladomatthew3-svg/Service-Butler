...
async function BillingPage({ searchParams }) {
    ...
    const disabled = (0, billing_mode_1.isBillingDisabled)();
    return (<div>
      <h1 className="dashboard-page-title text-semantic-text">Billing</h1>

      {disabled && (<div className="panel mt-4 border border-semantic-border bg-semantic-surface2 text-semantic-text">
          Billing disabled in MVP.
        </div>)}

      {!disabled && params.success === "1" && (<div className="panel mt-4 border border-semantic-border bg-semantic-surface2 text-semantic-text">
          Subscription checkout completed. Stripe webhook will sync subscription status shortly.
        </div>)}

      {!disabled && params.canceled === "1" && (<div className="panel mt-4 border border-semantic-border bg-semantic-surface2 text-semantic-text">
