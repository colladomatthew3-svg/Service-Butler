...
test("v2 scoring returns bounded explainable scores", () => {
  ...
  expect(result.jobLikelihoodScore).toBeGreaterThanOrEqual(0);
  expect(result.jobLikelihoodScore).toBeLessThanOrEqual(100);
  expect(result.contactabilityScore).toBeGreaterThanOrEqual(0);
  expect(result.contactabilityScore).toBeLessThanOrEqual(100);
  expect(["low", "medium", "high", "enterprise"]).toContain(result.revenueBand);
  expect(result.explainability.source_type).toBe("weather_alert");
});

test("v2 scoring escalates revenue band for stronger job signals", () => {
