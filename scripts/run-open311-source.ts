...
async function main() {
  ...
  const { data: sourceRow, error: sourceError } = await supabase
    ...
    .eq("source_type", "open311")
    .limit(1)
    .maybeSingle();

  if (sourceError) throw sourceError;
  if (!sourceRow?.id) throw new Error("Open311 source not found");

  const runResult = await runDataSourceConnector({
    supabase,
    tenantId,
    sourceId: String(sourceRow.id),
    actorUserId: "codex-open311-tranche"
  });
  ...
  const open311Candidates = ((opportunities || []) as Array<Record<string, unknown>>)
    .map((opportunity) => {
    ...
    .filter((candidate) => (candidate.source_type || "").toLowerCase().includes("open311"));

  const blockedReasonCounts = open311Candidates.reduce<Record<string, number>>((acc, candidate) => {
    ...
    JSON.stringify(
      {
        ...
        source_summary: runResult.sourceSummary
          ...
          : null,
        candidates: {
          total_open311_candidates: open311Candidates.length,
          dispatchable: open311Candidates.filter((candidate) => candidate.dispatch_eligible).length,
