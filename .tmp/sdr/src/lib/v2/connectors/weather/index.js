...
exports.weatherConnector = void 0;
...
function timeoutFetch(url, timeoutMs = 9000) {
...
function inferWeatherCategory(event) {
    const text = event.toLowerCase();
    if (text.includes("hail"))
        return "hail";
    if (text.includes("wind") || text.includes("tornado"))
        return "wind";
    if (text.includes("freeze") || text.includes("ice") || text.includes("cold"))
        return "freeze";
...
function categoryServiceLines(category) {
    if (category === "hail")
    ...
    if (category === "storm")
        return ["restoration", "roofing"];
    return ["general"];
}
function likelyJobType(category) {
    if (category === "hail" || category === "wind")
    ...
    if (category === "storm")
        return "storm restoration";
    return "weather risk inspection";
}
function eventTypeFromCategory(category) {
    if (category === "hail")
        return "weather_hail_alert";
    if (category === "wind")
        return "weather_wind_alert";
    if (category === "freeze")
        return "weather_freeze_alert";
    if (category === "flood")
        return "weather_flood_alert";
    if (category === "storm")
        return "weather_storm_alert";
    return "weather_alert";
}
exports.weatherConnector = {
    key: "weather.noaa",
    async pull(input) {
    ...
    classify(event) {
        ...
        if (category === "hail" || category === "wind")
            return { opportunityType: "roof_damage_risk", serviceLine: "roofing" };
        if (category === "storm")
            return { opportunityType: "storm_restoration", serviceLine: "restoration" };
    ...
    async healthcheck(input) {
        ...
        if (!res?.ok)
            return { ok: false, latencyMs, detail: "NOAA request failed" };
