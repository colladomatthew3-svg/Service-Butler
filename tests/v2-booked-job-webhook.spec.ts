...
type MockState = {
  ...
  opportunitySourceEventId: string | null;
  webhookInsertCount: number;
  jobUpsertCount: number;
  attributionUpsertCount: number;
  opportunityUpdateCount: number;
  auditInsertCount: number;
  lastAttributionPayload: Record<string, unknown> | null;
};
...
test("booked job webhook maps lead to opportunity attribution", async () => {
  ...
  expect(result.attribution.primaryOpportunityId).toBe("opp-1");
  expect(result.attribution.sourceEventId).toBe("src-1");
  expect(state.webhookInsertCount).toBe(1);
  expect(state.jobUpsertCount).toBe(1);
  expect(state.attributionUpsertCount).toBe(1);
  expect(state.opportunityUpdateCount).toBe(1);
  expect(state.auditInsertCount).toBe(1);
  expect(state.lastAttributionPayload?.primary_opportunity_id).toBe("opp-1");
});

test("booked job webhook without match keeps attribution nullable", async () => {
  const { client, state } = createSupabaseMock({
  ...
  expect(result.duplicate).toBeFalsy();
  expect(result.attribution.primaryOpportunityId).toBeNull();
  expect(result.attribution.sourceEventId).toBeNull();
  expect(state.attributionUpsertCount).toBe(1);
  expect(state.opportunityUpdateCount).toBe(0);
});

test("duplicate webhook short-circuits writes", async () => {
  const { client, state } = createSupabaseMock({
  ...
  expect(state.webhookInsertCount).toBe(1);
  expect(state.jobUpsertCount).toBe(0);
  expect(state.attributionUpsertCount).toBe(0);
