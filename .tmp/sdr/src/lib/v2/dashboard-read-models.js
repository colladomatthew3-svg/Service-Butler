...
exports.getCorporateDashboardReadModel = getCorporateDashboardReadModel;
exports.getFranchiseDashboardReadModel = getFranchiseDashboardReadModel;
...
function sourceCategoryFromSignal(sourceType) {
    ...
    if (normalized.includes("weather") || normalized.includes("storm") || normalized.includes("hail") || normalized.includes("wind") || normalized.includes("freeze") || normalized.includes("flood"))
...
function collectSourceTypes(explainability) {
...
function isDistressOpportunity(row) {
    const opportunityType = String(row.opportunity_type || "").toLowerCase();
    if (opportunityType.includes("distress"))
        return true;
...
async function getCorporateDashboardReadModel({ supabase, enterpriseTenantId }) {
    ...
    const [{ data: opportunities }, { data: assignments }, { data: jobs }, { data: connectorRuns }] = await Promise.all([
        franchiseIds.length
            ? supabase
                ...
                .select("tenant_id,opportunity_type,service_line,incident_cluster_id,routing_status,lifecycle_status,catastrophe_linkage_score,explainability_json,created_at")
    ...
    const jobRows = (jobs || []);
    const connectorRows = (connectorRuns || []);
    ...
    const sourceMixByCategory = oppRows.reduce((acc, row) => {
        const explainability = asRecord(row.explainability_json);
    ...
    const opportunitiesByServiceLine = oppRows.reduce((acc, row) => {
    ...
    const byFranchise = (franchises || []).map((franchise) => {
...
async function getFranchiseDashboardReadModel({ supabase, franchiseTenantId }) {
    const [{ data: opportunities }, { data: assignments }, { data: jobs }, { data: outreachEvents }, { data: territories }, { data: dataSources }, { data: connectorRuns }] = await Promise.all([
        supabase
            .from("v2_opportunities")
            .select("id,title,opportunity_type,service_line,postal_code,urgency_score,job_likelihood_score,catastrophe_linkage_score,incident_cluster_id,explainability_json,routing_status,lifecycle_status,created_at")
    ...
    const activeOpportunities = oppRows.filter((row) => !["booked_job", "closed_lost"].includes(String(row.lifecycle_status || "")));
    ...
    const jobsBookedToday = jobRows.filter((row) => {
        if (!String(row.status || "").includes("book"))
            return false;
        if (!row.booked_at)
            return false;
        return new Date(String(row.booked_at)).getTime() >= startOfDay.getTime();
    });
    const outreachActivity = outreachRows.reduce((acc, row) => {
        ...
        if (eventType === "sent" || eventType === "delivered")
            acc.by_channel[channel].sent += 1;
        if (eventType === "replied")
            acc.by_channel[channel].replied += 1;
        if (eventType === "failed")
            acc.by_channel[channel].failed += 1;
        if (eventType === "skipped")
            acc.by_channel[channel].skipped += 1;
        return acc;
    }, { total: outreachRows.length, by_channel: {} });
    ...
    for (const assignment of assignmentRows) {
        ...
        const matchedTerritory = (metadataTerritoryId ? territoriesById.get(metadataTerritoryId) : null) ||
            (opportunityId ? getTerritoryFromOpportunityPostal(opportunityId) : null);
        if (!matchedTerritory?.id)
            continue;
        const territoryId = String(matchedTerritory.id);
        const current = territoryPerformanceMap.get(territoryId) ||
            {
