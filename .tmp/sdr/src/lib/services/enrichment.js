...
exports.enrichOpportunityLive = enrichOpportunityLive;
exports.getEnrichmentProvider = getEnrichmentProvider;
...
async function geocodeWithCensus(address, city, state, postalCode) {
...
async function fetchZipMedianHomeValue(postalCode) {
...
function neighborhoodFromAddress(address, city) {
...
function buildUsgsAerialImageUrl(lat, lon) {
...
async function enrichOpportunityLive(input) {
...
async function fetchPremiumEnrichment(input) {
    ...
    try {
        ...
        if (!res.ok)
            return null;
        return normalizePremiumEnrichmentResponse((await res.json()), input);
    }
    catch {
        return null;
    }
...
function normalizePremiumEnrichmentResponse(payload, input) {
    ...
    return {
        ...
        notes: Array.isArray(payload.notes)
            ? payload.notes.map((note) => cleanString(note)).filter((note) => Boolean(note))
            : []
    };
}
function mergeEnrichment(base, premium) {
    return {
...
function getEnrichmentProvider(mode) {
    if (mode === "demo")
        return demoProvider;
    return null;
}
