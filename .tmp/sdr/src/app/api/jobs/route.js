...
async function POST(req) {
    ...
    if (!body.lead_id) {
        return server_1.NextResponse.json({ error: "lead_id is required" }, { status: 400 });
    }
    const pipeline = VALID_PIPELINE.includes((body.pipeline_status || "NEW"))
    ...
    if (insertError || !inserted) {
        return server_1.NextResponse.json({ error: insertError?.message || "Could not create job" }, { status: 400 });
    }
    ...
    await supabase
        ...
        .eq("account_id", accountId)
        .eq("id", body.lead_id);
    return server_1.NextResponse.json({ jobId: inserted.id }, { status: 201 });
}
