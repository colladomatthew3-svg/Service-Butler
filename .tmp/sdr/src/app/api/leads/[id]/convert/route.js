...
async function POST(_, { params }) {
    ...
    if ((0, review_mode_1.isDemoMode)()) {
        const result = (0, store_1.convertDemoLeadToJob)(id);
        if (!result) {
    ...
    if (lead.converted_job_id) {
        return server_1.NextResponse.json({ jobId: lead.converted_job_id, created: false });
    }
    ...
    if (relation?.job_id) {
        ...
        return server_1.NextResponse.json({ jobId: relation.job_id, created: false });
    }
    ...
    const { data: job, error: jobError } = await supabase
        ...
        .single();
    if (jobError || !job) {
        return server_1.NextResponse.json({ error: jobError?.message || "Could not convert lead" }, { status: 400 });
    }
    ...
    await supabase
        ...
        .eq("account_id", accountId)
        .eq("id", id);
    return server_1.NextResponse.json({ jobId: job.id, created: true });
}
