...
function checkAppUrl(): ReadinessCheck {
...
function checkDemoModeSafety(): ReadinessCheck {
...
function checkSupabaseEnv(): ReadinessCheck {
  const requiredKeys = [
...
async function checkSupabaseAuthHealth(): Promise<ReadinessCheck> {
...
async function checkSupabaseRestAccess(): Promise<ReadinessCheck> {
  const baseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
...
function checkBillingModeConfig(billingMode: ReturnType<typeof getBillingMode>): ReadinessCheck {
...
function checkStripeConfig(billingMode: ReturnType<typeof getBillingMode>): ReadinessCheck {
  if (billingMode !== "stripe") {
...
function checkEnrichmentConfig(): ReadinessCheck {
...
function checkEmailConfig(): ReadinessCheck {
...
function checkTwilioConfig(): ReadinessCheck {
...
function checkSmartleadConfig(): ReadinessCheck {
...
function checkInngestConfig(): ReadinessCheck {
...
function pass(name: string, required: boolean, message: string): ReadinessCheck {
  return { name, required, status: "pass", message };
}

function warn(name: string, required: boolean, message: string): ReadinessCheck {
  return { name, required, status: "warn", message };
}

function fail(name: string, required: boolean, message: string): ReadinessCheck {
