...
exports.usgsWaterConnector = void 0;
...
function classifyFlowCategory(cfs) {
    if (cfs >= 15_000)
        return "flood_indicator";
    if (cfs >= 6_000)
        return "high_water_indicator";
...
function flattenUsgsPayload(payload) {
...
exports.usgsWaterConnector = {
    ...
    async pull(input) {
        ...
        if (!siteCodes)
            return [];
        const url = `https://waterservices.usgs.gov/nwis/iv/?format=json&sites=${encodeURIComponent(siteCodes)}` +
