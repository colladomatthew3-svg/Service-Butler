...
exports.extractLeadContactCandidate = extractLeadContactCandidate;
exports.verifyLeadContactCandidate = verifyLeadContactCandidate;
...
function isPlaceholderPhone(value) {
    ...
    if (!digits)
        return true;
    if (digits.includes("5550") || digits.startsWith("1555") || digits.startsWith("555"))
        return true;
    if (/^(\d)\1+$/.test(digits))
        return true;
    return false;
...
function extractLeadContactCandidate(input) {
    const normalized = (input.sourceEvent?.normalized_payload || {}) || {};
    const raw = (input.sourceEvent?.raw_payload || {}) || {};
...
function verifyLeadContactCandidate(candidate, context) {
    const reasons = [];
    let score = 0;
