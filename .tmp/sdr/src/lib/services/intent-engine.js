...
function urgencyScore(timeframe) {
    const value = (timeframe || "").toLowerCase();
    if (!value)
        return 45;
    if (value.includes("asap") || value.includes("today") || value.includes("urgent"))
        return 88;
...
function generateSignals({ lead, forecast }) {
    ...
    const urgencyKeyword = service.includes("roof") ? "storm damage" :
        service.includes("plumb") ? "flood" :
            service.includes("hvac") ? "no cool" :
                "urgent repair";
    const urgency = urgencyScore(timeframe);
    const weatherBase = service.includes("roof") || service.includes("restor") ? 58 + weatherPrecip * 0.45 + weatherWind * 0.35 :
    ...
    const localDemand = service.includes("hvac") ? 74 :
            ...
            service.includes("plumb") ? 67 :
                service.includes("elect") ? 66 :
                    service.includes("land") ? 63 :
                        62;
    const seasonality = service.includes("hvac") ? 76 :
                ...
                service.includes("plumb") ? 69 :
                    service.includes("electrical") ? 60 :
                        service.includes("land") ? 64 :
                            58;
    const propertyContext = service.includes("roof") || service.includes("restor") ? 75 :
        service.includes("plumb") ? 69 :
            service.includes("elect") ? 66 :
