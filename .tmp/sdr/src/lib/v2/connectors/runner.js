...
exports.connectorRunnerInternals = void 0;
exports.runConnectorForSource = runConnectorForSource;
...
function toBoundedScore(value, fallback = 50) {
    const n = Number(value);
    if (!Number.isFinite(n))
        return fallback;
    return Math.max(0, Math.min(100, Math.round(n)));
}
function deriveFreshnessScore(occurredAt) {
...
function parsePostalFromText(text) {
    const match = text.match(/\b\d{5}\b/);
    return match?.[0] || "";
}
function parseLatLngFromPoint(point) {
    if (!point)
        return null;
...
function responseWindowFromUrgency(urgency) {
    if (urgency >= 85)
        return "0-4h";
    if (urgency >= 65)
        return "4-24h";
...
function classifyLikelyJobType(opportunityType, primaryServiceLine) {
    ...
    if (normalized.includes("plumb") || normalized.includes("sewer") || normalized.includes("pipe"))
        return "emergency plumbing";
    if (normalized.includes("roof") || normalized.includes("hail") || normalized.includes("wind"))
        return "roof damage inspection";
    if (normalized.includes("hvac") || normalized.includes("heat") || normalized.includes("ac"))
        return "HVAC outage";
    return "service dispatch";
}
function mapClusterType(eventCategory) {
    const category = eventCategory.toLowerCase();
    if (category.includes("storm") || category.includes("hail") || category.includes("wind") || category.includes("freeze"))
        return "storms";
    if (category.includes("fire"))
        return "fires";
    if (category.includes("flood") || category.includes("water"))
        return "flood_water";
    if (category.includes("infrastructure") || category.includes("utility") || category.includes("outage"))
        return "infrastructure";
    return "incident";
}
function scoreInputsForEvent(event, signalAgreement = 50) {
...
function ensureSourceMetadata({ event, connectorKey, sourceType, sourceName, complianceTermsStatus }) {
...
function validateNormalizedEvent(event) {
    const failures = [];
    if (!String(event.dedupeKey || "").trim())
        failures.push("dedupeKey missing");
    if (!String(event.eventType || "").trim())
        failures.push("eventType missing");
    if (!String(event.title || "").trim())
        failures.push("title missing");
    if (!String(event.occurredAt || "").trim())
        failures.push("occurredAt missing");
...
async function upsertIncidentClusterFromEvent({ supabase, tenantId, event }) {
...
async function resolveOpportunityCandidate({ supabase, tenantId, serviceLine, postalCode }) {
    let query = supabase
...
function mergeOpportunityScores({ existing, incoming, incomingConfidence, sourceType }) {
    const explainability = (existing.explainability_json || {});
    ...
    const weightedAverage = (current, next) => Math.max(0, Math.min(100, Math.round((Number(current || 0) * priorSignalCount + next + agreementBoost) / nextSignalCount)));
    const confidenceScore = Math.max(0, Math.min(100, Math.round(((Number(explainability.confidence_score || incomingConfidence) * priorSignalCount + incomingConfidence) / nextSignalCount + agreementBoost) / 1.05)));
    return {
...
async function upsertOpportunityFromEvent({ supabase, tenantId, sourceEventId, event, classification, clusterId }) {
    const scoring = (0, scoring_1.computeOpportunityScores)(scoreInputsForEvent(event, 55));
    ...
    if (candidate?.id) {
        ...
        const { data: updated, error } = await supabase
            ...
            .single();
        if (error || !updated?.id)
            throw new Error(error?.message || "Failed to update v2 opportunity");
        ...
        const { data: inserted, error } = await supabase
            ...
            .single();
        if (error || !inserted?.id)
            throw new Error(error?.message || "Failed to create v2 opportunity");
    ...
    return {
        opportunityId,
        scoring: finalScores,
        multiSignal: Boolean(finalScores.explainability.multi_signal)
    };
}
exports.connectorRunnerInternals = {
...
async function runConnectorForSource({ supabase, tenantId, sourceId, sourceType, sourceConfig, actorUserId, connector }) {
    ...
    if (compliance.termsStatus === "blocked" || !compliance.ingestionAllowed) {
        const reason = compliance.termsStatus === "blocked"
    ...
    try {
        ...
        for (const event of normalizedEvents) {
            ...
            const locationPoint = toPoint(event.latitude, event.longitude);
            const eventScoring = (0, scoring_1.computeOpportunityScores)(scoreInputsForEvent(event, 50));
        ...
        await supabase
            ...
            metadata: {
                ...
                avg_data_freshness_score: normalizedEvents.length > 0 ? Math.round(totalFreshness / normalizedEvents.length) : 0,
                avg_source_reliability: normalizedEvents.length > 0 ? Math.round(totalReliability / normalizedEvents.length) : 0,
